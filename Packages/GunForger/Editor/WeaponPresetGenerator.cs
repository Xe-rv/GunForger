...
/// <summary>
/// Editor utility for generating pre-configured RangedWeapon prefabs (Pistol, Rifle, Shotgun, etc.)
/// Provides menu items under Assets/Create/2D Weapon System/Weapon Prefabs/...
/// Uses reflection to set private fields on RangedWeapon instances.
/// </summary>
public static class WeaponPrefabGenerator
{
    private const string MENU_PATH = "Assets/Create/2D Weapon System/Weapon Prefabs/";

    // Each MenuItem creates a configured prefab variant using CreateWeaponPrefab helper
    ...
    /// <summary>
    /// Core prefab creation: constructs GameObject, configures stats and saves the result as a prefab asset.
    /// </summary>
    private static void CreateWeaponPrefab(string weaponName, System.Action<RangedWeapon> configure, Color weaponColor)
    {
        ...
        // Apply default name and then call configuration callback (uses reflection to set fields)
        SetField(rangedWeapon, "weaponName", weaponName);
        configure(rangedWeapon);

        // Determine path and save prefab
    ...
    /// <summary>
    /// Uses reflection to set private fields on RangedWeapon instances. Logs a warning if field not found.
    /// </summary>
